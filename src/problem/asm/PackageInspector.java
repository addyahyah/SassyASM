...
public class PackageInspector {
	// String sourceDir = "C:\\EclipseWorkspaces\\CSSE374\\SassyASM\\src";
	...
	private static ArrayList<String> toArrayHelper(String sourceDir, File dir) {
		...
		for (String name : dir.list()) {
			...
			if (isJavaClass(name)) {
				// TODO FIXME: this is weird a weird fix, maybe there is
				// another way to remove previous patterns
				int index = dir.getPath().lastIndexOf(sourceDir)
						+ sourceDir.length();
				classList.add(dir.getPath().substring(index).replace("\\", ".")
						.substring(1)
